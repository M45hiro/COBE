...
class Illumination_Estimator(nn.Module):
    ...
    def forward(self, img):
        ...
        # 计算illumination prior
        # stx()
        input = torch.cat([img, mean_c], dim=1)
        # 论文中的C步骤，channel-wise concatenation
...
class IG_MSA(nn.Module):
    def __init__(
        ...
        Args:
            dim:维度
            dim_head:指每个注意力头的维度大小
            heads:模型中注意力头的数量
            在自注意力机制（self-Attention）中，输入序列会被分为不同的注意力头，每个注意力头负责捕捉输入
            序列中的不同方面的关系和模式
            dim_head表示每个注意力头的维度，决定了每个头可以学习和关注的特征的数量
            heads则代表了整个模型中并行运行的注意力头的数量
            增加注意力头的数量可以更加充分的捕捉输入序列中的特征和模式，从而提升模型的表示能力和性能
        '''
        super().__init__()
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        # 归一化处理 将每个位置的注意力权重限制在0到1之间，确保总和为1，形成归一化的注意力权重
        x = attn @ v   # b,heads,d,hw
        # 用归一化的注意力权值对张量v进行加权求和
        x = x.permute(0, 3, 1, 2)    # Transpose
        # 为形状变换做准备
        x = x.reshape(b, h * w, self.num_heads * self.dim_head)
        # 变成batch_size, height*width, num_heads*head_dim的形式
        out_c = self.proj(x).view(b, h, w, c)
        # 通过线性投影操作对x线性变换，转换成原始形式
