...
from yolov8.plate_recognition.plate_rec import get_plate_result,init_model,cv_imread
from yolov8.plate_recognition.double_plate_split_merge import get_split_merge
from yolov8.fonts.cv_puttext import cv2ImgAddText
import base64
...
def four_point_transform(image, pts):
    '''
    四点透视变换，用于对图像进行透视变形，以得出一个新的视角
    输入参数：
    image 需进行透视变换的图像
    pts 图像上执行透视变换的四点
    '''
    rect = pts.astype('float32')
    # 四个变量分别表示：左上、右上、右下、左下
    (tl, tr, br, bl) = rect
    # 欧氏空间距离计算的公式
    ...
    # 创建新数组dst，表示透视变换后四点位置坐标
    dst = np.array([
        [0, 0],
        [maxWidth - 1, 0],
        [maxWidth - 1, maxHeight - 1],
        [0, maxHeight - 1]], dtype = "float32")
    # 确定透视变换矩阵M，用于将透视rect转换为透视dst
...
def my_nums(dets,iou_thresh):
    # 执行非极大值抑制(Non-Maximum Suppresion, NMS)操作
    # 去除那些与真实目标重合度高，但是置信度低的预测框，从而保留最优的预测结果
    y = dets.clone()
    y_box_score = y[:,:5]
...
parser.add_argument('--detect_model', nargs='+', type=str, default=r'yolov8/weights/yolov8s.pt', help='model.pt path(s)')  # yolov8检测模型
parser.add_argument('--rec_model', type=str, default=r'yolov8/weights/plate_rec_color.pth', help='model.pt path(s)') # 车牌字符识别模型
