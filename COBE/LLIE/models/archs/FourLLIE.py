...
class FourLLIE(nn.Module):
    def __init__(self, nf=64):
        ...
        self.fuseblock = FuseBlock(nf)
        self.fourget = arch_util.make_layer(ResidualBlock_noBN_f, 1)
        self.conv_16_64 = nn.Conv2d(in_channels=16, out_channels=
                                    64,kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        fea_fuse = self.fuseblock(spa=fea_light, fre=fea_unfold)
        fea_unfold = self.fourget(fea_unfold)
        fea_light = fea_fuse
        fea_fuse = self.fuseblock(spa=fea_light, fre=fea_unfold)
        fea_unfold = self.fourget(fea_unfold)
        fea_light = fea_fuse
